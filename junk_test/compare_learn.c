...
int main(void)
{
    ...
    float average = 0;
    printf("Please enter the number of boxes whose weight you want to calculate (Between 50 to 200) : ");
    scanf("%d", &num_boxes);
    while ((num_boxes < 50) || (num_boxes > 200))
    {
        printf("Please enter a value in the correct range : ");
        scanf("%d", &num_boxes);
    }
    weightcal(weights, &num_boxes, &total_weight);
    printArray(weights, num_boxes);
    printf("The total weight is: %.2f\n", total_weight);
    avgweight(&average, &total_weight, &num_boxes);
